...
/**
	 ...
	 * @return
             */
    public Map<String, Object> updateOrGetHoleScoreCardByGroupIdTime(Long matchId, Long groupId,Long createTime) throws UnsupportedEncodingException {
        Map<String, Object> result = new HashMap<>();
        MatchInfo matchInfo = matchDao.get(MatchInfo.class, matchId);
        result.put("matchInfo", matchInfo);
        //固定的首列：本组用户
        ...
        Map<String, Object> mapResult = null;
        if (userList != null && userList.size() > 0) {
            mapResult = getChengjiListTime(chengjiList,endTrChengji,userList0,userList1,groupId,matchInfo,result,createTime);
        }
        //成绩行
        result.put("chengjiList", mapResult.get("chengjiList"));
     ...
     * 把同组每个用户的比洞赛逐洞比分以及双方比洞赛的成绩结果放到result中
     */
    public Map<String, Object> getChengjiListTime(List<HoleMatchScoreResultBean> chengjiList,HoleMatchScoreResultBean endTrChengji,
                                              List<Map<String, Object>> userList0,List<Map<String, Object>> userList1,Long groupId, MatchInfo matchInfo, Map<String, Object> result,Long  createTime) {
        //第一对儿用户 得分
        Long userId0 = matchService.getLongValue(userList0.get(0), "uiId");
        String userName00 = matchService.getName(userList0.get(0), "uiRealName");
        Long teamId0 = matchService.getLongValue(userList0.get(0),"team_id");
        List<Map<String, Object>> uscoreList00 = matchDao.getScoreByUserIdTime(groupId, userId0, matchInfo, teamId0,createTime);
        for(int i=0;i<18;i++){
            uscoreList00.get(i).put("userId",userId0);//由于新记分卡在matchscore中没记录，查不到用户userId，在这里放进去
        }
        List<Map<String, Object>> uscoreList0 = new ArrayList<Map<String,Object>>();
        ...
        if(userList0.size() ==1 ){
            ...
            userName01 = matchService.getName(userList0.get(1), "uiRealName");
            teamId0 = matchService.getLongValue(userList0.get(1),"team_id");
            List<Map<String, Object>> uscoreList01 = matchDao.getScoreByUserIdTime(groupId, userId0, matchInfo, teamId0,createTime);
            result.put("uscoreList01", uscoreList01);
        ...
        if (userList1.size()>0) {
            ...
            String userName10 = matchService.getName(userList1.get(0), "uiRealName");
            Long teamId1 = matchService.getLongValue(userList1.get(0), "team_id");
            List<Map<String, Object>> uscoreList10 = matchDao.getScoreByUserIdTime(groupId, userId1, matchInfo, teamId1,createTime);
            for (int i = 0; i < 18; i++) {
                uscoreList10.get(i).put("userId", userId1);//由于新记分卡在matchscore中没记录，查不到用户userId，在这里放进去
            }
            result.put("uscoreList10", uscoreList10);
            ...
            if (userList1.size() == 1) {
                ...
                userName11 = matchService.getName(userList1.get(1), "uiRealName");
                teamId1 = matchService.getLongValue(userList1.get(1), "team_id");
                List<Map<String, Object>> uscoreList11 = matchDao.getScoreByUserIdTime(groupId, userId1, matchInfo, teamId1,createTime);
                result.put("uscoreList11", uscoreList11);
